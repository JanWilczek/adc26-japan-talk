...
## Motivation: APVTS is poor at creating presets
...
## Operations on all parameters using Visitor
...
## Transtion to type-erased parameters

1. How to transition to this approach.
    1. If you use `addToLayout<>()` approach, replace `addToLayout<>()` calls with `builder.add<>()` calls.
    1. If you use `APVTS::createAndAddParameter()` approach, replace `addToLayout<>()` calls with `builder.add<>()` calls.
    1. If you use APVTS, replace it with `ParameterHolder`, and use the described serialization approach.
    1. If you store parameter references and serialize them individually, keep them, and consider using ParameterHolder for operations on all parameters.
1. Use these features from `wolfsound-dsp-utils`
    1. `(Juce)ParameterHolder`
    1. Serialization function
    1. Deserialization function

## Summary

1. We can use Type Erasure to facilitate operations on all parameters (serialization, deserialization, presets)
1. Courses: DSP & JUCE
1. Contact
